...
export const HostrListingsSearchInputSchema = z.object({
  ...
  "rentOrBuy": z.string().describe("Filter by listing market mode. Use rent for listings with recurring prices and buy for fixed-price listings.").optional(),
...
export const HostrListingsCreateInputSchema = z.object({
  ...
  "minDuration": z.string().describe("ISO 8601 duration, e.g. P2D, P1W, P1M, or P1Y.").optional(),
  ...
  "maxDisputePeriod": z.number().int().min(0).describe("Maximum time in seconds after the order end date that escrow dispute resolution may remain open.").optional(),
  "cancellationPolicy": z.array(z.object({
  "secondsBeforeStart": z.number().int().min(0).optional(),
  "secondsAfterOrder": z.number().int().min(0).optional(),
  "refundFraction": z.number().min(0).max(1)
}).strict()).describe("Refund rules based on how far before the order start or after order creation the buyer cancels. Each rule must include secondsBeforeStart, secondsAfterOrder, or both.").optional(),
...
export const HostrListingsEditInputSchema = z.object({
  ...
  "patch": z.record(z.string(), z.unknown()).describe("Listing fields to change. Supports title, description, address, type, images, prices, specifications, guests, beds, bedrooms, bathrooms, active, negotiable, autoAccept, minDuration, quantity, securityDeposit, minPaymentAmount, maxDisputePeriod, and cancellationPolicy. patch.specifications uses the same canonical listing specifications/amenities map as hostr_listings_create; use wireless_internet for Wi-Fi/wifi/WIFI.").optional(),
...
export const HostrOrderBookAndPayInputSchema = z.object({
  "listingAnchor": z.string().describe("Naddr/a-tag anchor for the auto-accept listing."),
  "start": z.string().datetime().describe("Order start calendar date encoded as YYYY-MM-DDT00:00:00Z. Order start/end values are calendar dates, not timezone instants. Encode the requested date as YYYY-MM-DDT00:00:00Z; the trailing Z is storage syntax only. Do not convert from the user timezone, listing timezone, check-in time, or check-out time."),
  "end": z.string().datetime().describe("Order end calendar date encoded as YYYY-MM-DDT00:00:00Z. Order start/end values are calendar dates, not timezone instants. Encode the requested date as YYYY-MM-DDT00:00:00Z; the trailing Z is storage syntax only. Do not convert from the user timezone, listing timezone, check-in time, or check-out time."),
  "amount": z.object({
  "value": z.string().describe("Payment amount as a decimal string to avoid precision loss. If unit is sats, this value is a satoshi count, where 1 sat = 1/100,000,000 BTC."),
  "currency": z.string().describe("Currency or denomination, such as USD or BTC. For bitcoin amounts expressed in sats, use currency BTC with unit sats."),
  "unit": z.string().describe("Optional display/base unit. When unit is sats, sats means satoshis: 1 sat = 1/100,000,000 BTC. Do not interpret sats as whole BTC, cents, dollars, or any fiat subunit.").optional(),
  "decimals": z.number().int().min(0).describe("Optional decimal precision for raw-denomination amounts. For unit sats, use decimals 0 because satoshis are already the smallest bitcoin unit.").optional()
}).strict().describe("Optional order amount override. Must be at or above the listing price.").optional(),
  ...
  "proofTimeoutSeconds": z.number().int().min(30).max(3600).describe("Seconds to wait for the global order stream to emit the committed order after swap completion.").default(300).optional()
}).strict();

export const HostrOrdersOfferInputSchema = z.object({
  ...
  "tradeId": z.string().describe("Existing order trade id for a follow-up offer.").optional(),
  "start": z.string().datetime().describe("Order start calendar date for a first offer, encoded as YYYY-MM-DDT00:00:00Z. Order start/end values are calendar dates, not timezone instants. Encode the requested date as YYYY-MM-DDT00:00:00Z; the trailing Z is storage syntax only. Do not convert from the user timezone, listing timezone, check-in time, or check-out time.").optional(),
  "end": z.string().datetime().describe("Order end calendar date for a first offer, encoded as YYYY-MM-DDT00:00:00Z. Order start/end values are calendar dates, not timezone instants. Encode the requested date as YYYY-MM-DDT00:00:00Z; the trailing Z is storage syntax only. Do not convert from the user timezone, listing timezone, check-in time, or check-out time.").optional(),
  "amount": z.object({
  "value": z.string().describe("Payment amount as a decimal string to avoid precision loss. If unit is sats, this value is a satoshi count, where 1 sat = 1/100,000,000 BTC."),
  "currency": z.string().describe("Currency or denomination, such as USD or BTC. For bitcoin amounts expressed in sats, use currency BTC with unit sats."),
  "unit": z.string().describe("Optional display/base unit. When unit is sats, sats means satoshis: 1 sat = 1/100,000,000 BTC. Do not interpret sats as whole BTC, cents, dollars, or any fiat subunit.").optional(),
  "decimals": z.number().int().min(0).describe("Optional decimal precision for raw-denomination amounts. For unit sats, use decimals 0 because satoshis are already the smallest bitcoin unit.").optional()
}).strict().describe("Optional order amount override. Omit to use listing price rules.").optional(),
...
export const HostrOrderTradeInputSchema = z.object({
  "tradeId": z.string(),
  "amount": z.object({
  "value": z.string().describe("Payment amount as a decimal string to avoid precision loss. If unit is sats, this value is a satoshi count, where 1 sat = 1/100,000,000 BTC."),
  "currency": z.string().describe("Currency or denomination, such as USD or BTC. For bitcoin amounts expressed in sats, use currency BTC with unit sats."),
...
export const HostrOrderPayInputSchema = z.object({
  "tradeId": z.string().describe("Order trade id from negotiation updates."),
  "escrowServiceId": z.string().describe("Optional escrow service id/pubkey/contract address. Omit to use the first compatible mutual escrow.").optional(),
  "dryRun": z.boolean().default(true).optional(),
  "timeoutSeconds": z.number().int().min(1).max(60).default(12).optional()
}).strict();

export const HostrOrderCommitInputSchema = z.object({
  "swapId": z.string().describe("Boltz swap id returned by hostr_orders_pay."),
  "dryRun": z.boolean().default(true).optional(),
  "timeoutSeconds": z.number().int().min(1).max(60).default(12).optional()
}).strict();

export const HostrOrderReviewInputSchema = z.object({
  "tradeId": z.string().describe("Order trade id for the trip to review."),
...
export const HostrEscrowInvolveInputSchema = z.object({
  "tradeId": z.string().describe("Required order trade id. Escrow messages cannot be sent without a trade id because the thread must include buyer, seller, and escrow."),
...
export const HostrOrderCollectionInputSchema = z.object({
  "limit": z.number().int().min(1).max(200).default(50).optional(),
  "tradeId": z.string().describe("Optional order trade id to look up directly after payment/proof completion.").optional(),
  "waitSeconds": z.number().int().min(0).max(300).describe("How long to poll for a committed public order when tradeId is provided.").default(15).optional()
}).strict();
...
export const HostrEscrowServiceUpdateInputSchema = z.object({
  ...
  "fee": z.object({
  "ppm": z.number().int().min(0).optional(),
  "base": z.string().optional(),
  "min": z.string().optional(),
  "max": z.string().optional(),
  "assetOverrides": z.record(z.string(), z.unknown()).optional()
}).strict().describe("Full replacement escrow fee policy. ppm is parts per million; base, min, and max are asset smallest-unit strings. assetOverrides is keyed by token contract address or \"native\".").optional(),
...
export const HostrEscrowTradeViewInputSchema = z.object({
  "tradeId": z.string().describe("Hostr order trade id to inspect.")
}).strict();

export const HostrEscrowTradeAuditInputSchema = z.object({
  "tradeId": z.string().describe("Hostr order trade id to audit.")
}).strict();

export const HostrEscrowArbitrateInputSchema = z.object({
  "tradeId": z.string().describe("Hostr order trade id to arbitrate."),
  "paymentForward": z.number().min(0).max(1).describe("Fraction of the escrowed payment to forward to the seller, from 0 to 1."),
  "bondForward": z.number().min(0).max(1).describe("Fraction of the escrow bond to forward according to the contract settlement rules, from 0 to 1."),
...
export const HostrSwapsWatchInputSchema = z.object({
  "swapId": z.string(),
  "tradeId": z.string().describe("Optional order trade id used to fall back to public order lookup after the swap completes or is not found.").optional(),
  "orderWaitSeconds": z.number().int().min(0).max(60).describe("How long to poll for the committed order after proof completion or swap-not-found fallback. Keep short; capped below MCP client timeouts.").default(20).optional()
}).strict();
...
export const hostrActionCatalog = [
  ...
  },
  {
    ...
    toolName: "hostr_listings_orderGroups",
    title: "Fetch Listing Order Groups",
    description: "Fetch public order groups for one or more listings. Use this before availability-sensitive order workflows when the agent needs to explain conflicts.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nRead-only behavior: this tool retrieves or analyzes Hostr state and is safe to call when the user asks to inspect, search, explain, debug, or choose the next action. Prefer read tools before write tools when the user intent is ambiguous or when you need concrete listing, trade, thread, or profile ids.\n\nUse when the user asks why dates are unavailable, wants booking history/conflicts for a listing, or needs order context before changing availability-sensitive plans.",
    inputTypeName: "HostrListingsAnchorsInput",
    inputJsonSchema: {
    "type": "object",
    "additionalProperties": false,
  ...
  },
  {
    ...
    toolName: "hostr_orders_bookAndPay",
    title: "Start Order Payment",
    description: "Use this foreground handoff tool whenever the user says to book, reserve, make, or create a order for an auto-accept listing at or above the listed price. It creates the private order offer and escrow funding swap. Order start/end values are calendar dates, not timezone instants. Encode the requested date as YYYY-MM-DDT00:00:00Z; the trailing Z is storage syntax only. Do not convert from the user timezone, listing timezone, check-in time, or check-out time. Order privacy: Hostr intentionally publishes committed orders and escrow trades under Hostr-created per-trade temporary pubkeys rather than the active account pubkey. This is normal and preserves user privacy; never present a different order buyer pubkey as an identity mismatch. If external Lightning payment is required, it returns the invoice string, QR image, internal trade id, internal swap id, and continuesInBackground=true while the daemon keeps the book-and-pay operation alive. CRITICAL UI REQUIREMENT: leave only the QR image and invoice text visibly in the answer to the user; do not show internal trade id or swap id in the payment prompt, and do not replace the payment prompt with a summary. The next assistant action after rendering that visible payment prompt must be the read-only `hostr_swaps_watch` with the returned `swapId`, `tradeId`, and `orderWaitSeconds`; do not stop after displaying the invoice or wait for the user to say they paid. The returned `orderWaitSeconds` is short and capped below MCP client timeouts; do not substitute a longer proof timeout. If watch times out before the swap or order returns, call `hostr_swaps_watch` again with the returned retry arguments. When watch completes or cannot find the swap, query `hostr_trips_list` with `tradeId` until the committed order appears, then show a order card. Do not call `hostr_orders_commit`; proof publication is owned by the global Hostr payment proof orchestrator.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nOrder date rule: Hostr order start/end inputs are calendar dates, not timezone instants. Preserve the dates the user requested and encode them as YYYY-MM-DDT00:00:00Z. The trailing Z is storage syntax only; do not convert from the user timezone, listing timezone, El Salvador time, check-in time, or check-out time.\n\nPrimary guest booking flow: use this when the user says book, reserve, make a order, create a order, or otherwise clearly wants an auto-accept stay at or above the listed price. If the user says guest, my guest account, my trip, or similar, make sure the active account is the guest account first by using session account tools; do not book from an unrelated host or escrow account just because it is already active. It creates the private offer, prepares escrow funding, returns external Lightning payment details when needed, and keeps the daemon-side book-and-pay operation alive. The committed order and escrow trade are intentionally published under Hostr-created per-trade temporary pubkeys for privacy, so the buyer/order pubkey may differ from the active logged-in Hostr account. Treat that as normal and never describe it as an identity mismatch. If invoice/QR are returned, show only the invoice string and QR image visibly in the payment prompt; keep internal tradeId and swapId hidden from the user-facing payment message. The next assistant action after rendering the payment prompt must be hostr_swaps_watch with swapId, tradeId, and orderWaitSeconds to monitor payment/proof/order completion. Do not stop after displaying the invoice or wait for the user to say they paid. orderWaitSeconds is intentionally short and capped below MCP client timeouts; if watch times out before the swap or order returns, call hostr_swaps_watch again with the returned retry arguments. When watch completes or cannot find the swap, call hostr_trips_list with the same tradeId until the committed order appears. Do not call hostr_orders_commit for this normal path; proof publication is owned by the global payment proof orchestrator.",
    ...
    inputSchema: HostrOrderBookAndPayInputSchema,
    readOnly: false,
  },
  {
    id: "hostr.orders.negotiateOffer",
    toolName: "hostr_orders_negotiateOffer",
    title: "Create Order Negotiation Offer",
    description: "Create only a private negotiate-stage order offer. Use this for explicit negotiation/counteroffer requests, not for user intents like \"book\", \"reserve\", \"make a order\", or auto-accept at the listed price; those must use hostr_orders_bookAndPay instead.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nPreview rule: dryRun defaults to true. First call with dryRun=true, show the user the returned preview or card, and only repeat with dryRun=false after the user explicitly approves that preview in the conversation. Do not treat vague acknowledgement as approval for destructive, payment, publication, messaging, cancellation, recovery, or arbitration actions.\n\nOrder date rule: Hostr order start/end inputs are calendar dates, not timezone instants. Preserve the dates the user requested and encode them as YYYY-MM-DDT00:00:00Z. The trailing Z is storage syntax only; do not convert from the user timezone, listing timezone, El Salvador time, check-in time, or check-out time.\n\nNegotiation-only flow: use for explicit offers, counteroffers, price/date negotiation, or non-auto-accept order proposals. Do not use this for straightforward \"book/reserve\" intents on auto-accept listings; use hostr_orders_bookAndPay there. Preview with dryRun=true, then send the private negotiation event with dryRun=false only after approval.",
    ...
    inputSchema: HostrOrdersOfferInputSchema,
    readOnly: false,
  },
  {
    id: "hostr.orders.negotiateAccept",
    toolName: "hostr_orders_negotiateAccept",
    title: "Accept Order Negotiation",
    description: "Accept the latest private negotiate-stage order offer in a trade thread by replying with a matching negotiate-stage event.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nPreview rule: dryRun defaults to true. First call with dryRun=true, show the user the returned preview or card, and only repeat with dryRun=false after the user explicitly approves that preview in the conversation. Do not treat vague acknowledgement as approval for destructive, payment, publication, messaging, cancellation, recovery, or arbitration actions.\n\nUse when the user wants to accept the latest private negotiated offer in a known trade thread. If tradeId is unknown, call hostr_updates, hostr_thread_view, hostr_trips_list, or hostr_bookings_list first to identify the trade.",
    ...
    inputSchema: HostrOrderTradeInputSchema,
    readOnly: false,
  },
  {
    id: "hostr.orders.pay",
    toolName: "hostr_orders_pay",
    title: "Pay Order Offer",
    description: "Preview or create the escrow funding swap for a payable order trade. The live action sends the escrow selection into the private thread as an unsigned child event, prepares the escrow fund calls, creates the Boltz swap invoice, and persists the payment context for commit.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nPreview rule: dryRun defaults to true. First call with dryRun=true, show the user the returned preview or card, and only repeat with dryRun=false after the user explicitly approves that preview in the conversation. Do not treat vague acknowledgement as approval for destructive, payment, publication, messaging, cancellation, recovery, or arbitration actions.\n\nManual recovery/debug payment flow only. Normal AI-initiated auto-accept payment should use hostr_orders_bookAndPay. Use this when a negotiated or partially completed trade already exists and the user explicitly wants to create or inspect escrow funding for that trade.",
    ...
    inputSchema: HostrOrderPayInputSchema,
    readOnly: false,
  },
  {
    id: "hostr.orders.commit",
    toolName: "hostr_orders_commit",
    title: "Commit Paid Order",
    description: "Preview or publish the public commit-stage order after the escrow funding swap has completed and produced a claim transaction proof.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nPreview rule: dryRun defaults to true. First call with dryRun=true, show the user the returned preview or card, and only repeat with dryRun=false after the user explicitly approves that preview in the conversation. Do not treat vague acknowledgement as approval for destructive, payment, publication, messaging, cancellation, recovery, or arbitration actions.\n\nManual recovery/debug commit flow only. Do not use after hostr_orders_bookAndPay; that path relies on the global payment proof orchestrator. Use only when a swap proof already exists for a trade and the user explicitly needs to preview or publish the public commit-stage order.",
    ...
    inputSchema: HostrOrderCommitInputSchema,
    readOnly: false,
  },
  {
    id: "hostr.orders.cancel",
    toolName: "hostr_orders_cancel",
    title: "Cancel Order",
    description: "Cancel either the private negotiate-stage order for a trade, or the committed public order if one exists.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nPreview rule: dryRun defaults to true. First call with dryRun=true, show the user the returned preview or card, and only repeat with dryRun=false after the user explicitly approves that preview in the conversation. Do not treat vague acknowledgement as approval for destructive, payment, publication, messaging, cancellation, recovery, or arbitration actions.\n\nUse to cancel a private negotiation or committed order for a concrete trade. If tradeId is unclear, inspect updates, trips, bookings, or thread view first. Preview the cancellation and send with dryRun=false only after explicit approval.",
    inputTypeName: "HostrOrderTradeInput",
    inputJsonSchema: {
    "type": "object",
    "additionalProperties": false,
  ...
  },
  {
    ...
    toolName: "hostr_orders_review",
    title: "Review Order",
    description: "Preview or publish a guest review for a committed Hostr order. This creates a kind 31555 Nostr review with raw text content, a normalized rating tag, and a participation proof tag for the order trade; preview first and only publish after explicit approval.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nPreview rule: dryRun defaults to true. First call with dryRun=true, show the user the returned preview or card, and only repeat with dryRun=false after the user explicitly approves that preview in the conversation. Do not treat vague acknowledgement as approval for destructive, payment, publication, messaging, cancellation, recovery, or arbitration actions.\n\nUse when the guest wants to leave a rating/review for a completed or confirmed Hostr trip. If tradeId is unclear, inspect hostr_trips_list first. Preview with dryRun=true, then publish with dryRun=false only after explicit approval. Skip this action if no committed order can be found for the trade.",
    ...
    inputSchema: HostrOrderReviewInputSchema,
    readOnly: false,
  },
  {
    id: "hostr.updates",
  ...
  },
  {
    ...
    description: "Open or message the escrow conversation for a specific order trade, then return the fixed thread-view contract. This action always requires tradeId and always resolves the trade buyer, seller, and escrow participants into one shared trade thread; it must not create an escrow-only side conversation. If content is omitted, show the escrow trade thread and ask the user what to message the escrow. If content is provided, preview by default and send only with dryRun false.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nWrite behavior: this tool can create, publish, send, recover, delete, pay, arbitrate, or otherwise change state outside ChatGPT. Explain the important effect to the user before live execution, preserve user-visible previews, and require explicit user approval before any irreversible or externally visible action.\n\nPreview rule: dryRun defaults to true. First call with dryRun=true, show the user the returned preview or card, and only repeat with dryRun=false after the user explicitly approves that preview in the conversation. Do not treat vague acknowledgement as approval for destructive, payment, publication, messaging, cancellation, recovery, or arbitration actions.\n\nUse when the user explicitly asks to involve/message escrow for a specific order trade. Always pass tradeId. This opens the shared buyer/seller/escrow trade thread; never create an escrow-only side conversation. If no message content is provided, show the thread and ask what to send.",
  ...
  },
  {
    ...
    description: "List order groups involving the authenticated user as guest from the live userSubscriptions.myResolvedTripsList replay. Committed orders may be authored by Hostr-created per-trade temporary pubkeys for privacy; treat those temporary pubkeys as expected order accounts for the active user, not as an identity mismatch. Do not perform fresh order-by-author Nostr queries for this view. Return the fixed trip-card display contract with resolved participant profile names. Cancelled trip cards must preserve a bold Cancelled marker. Pass `tradeId` after a book-and-pay swap watch completes or cannot find the swap to wait briefly for the committed public order and return it for display.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nRead-only behavior: this tool retrieves or analyzes Hostr state and is safe to call when the user asks to inspect, search, explain, debug, or choose the next action. Prefer read tools before write tools when the user intent is ambiguous or when you need concrete listing, trade, thread, or profile ids.\n\nUse for guest-side orders: \"my trips\", \"my bookings as guest\", \"my order reference\", \"did my order complete\", \"my host has not replied\", or after book-and-pay/swap watch with tradeId to wait for the committed order card. Do not use this as the first monitor immediately after a payment-required hostr_orders_bookAndPay result; first call hostr_swaps_watch with the required next-tool arguments, then use trips once the swap watch resolves or reports the order is pending. Trip cards may resolve committed orders authored by Hostr-created per-trade temporary pubkeys; this is expected privacy behavior for the active account, not a mismatch. Do not perform fresh order-by-author Nostr queries for this view.",
    ...
    inputSchema: HostrOrderCollectionInputSchema,
    readOnly: true,
  },
  {
    ...
    description: "List order groups where the authenticated user is the host from the live userSubscriptions.myResolvedHostingsList replay. Do not perform fresh Nostr listing/order-by-author queries for this view. Return the fixed hosting-card display contract with resolved participant profile names, including \"Hosting {guest} at: {stay}\" text.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nRead-only behavior: this tool retrieves or analyzes Hostr state and is safe to call when the user asks to inspect, search, explain, debug, or choose the next action. Prefer read tools before write tools when the user intent is ambiguous or when you need concrete listing, trade, thread, or profile ids.\n\nUse for host-side orders on listings authored by the authenticated user: \"my bookings\", \"who booked my place\", \"hosting orders\", or host calendar context. Do not use this as the first monitor immediately after a payment-required guest booking; hostr_swaps_watch comes first. Do not perform fresh order-by-author Nostr queries for this view.",
    inputTypeName: "HostrOrderCollectionInput",
    inputJsonSchema: {
    "type": "object",
    "additionalProperties": false,
  ...
  },
  {
    ...
    description: "Escrow-only tool. View the on-chain state, event history, and Hostr order context for a trade assigned to the authenticated escrow pubkey.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nEscrow role notes: this tool is visible only when the authenticated Hostr pubkey is configured as an escrow service. It is for escrow-operator work, not ordinary guest/host booking flows. Keep user profile edits in hostr_profile_edit; escrow service tools only manage escrow service events/settings.\n\nRead-only behavior: this tool retrieves or analyzes Hostr state and is safe to call when the user asks to inspect, search, explain, debug, or choose the next action. Prefer read tools before write tools when the user intent is ambiguous or when you need concrete listing, trade, thread, or profile ids.\n\nEscrow-operator trade detail: inspect on-chain state, event history, participants, amounts, and order context for a trade before audit or arbitration.",
  ...
  },
  {
    ...
    description: "Escrow-only tool. Run a structured order and transition audit for a Hostr trade assigned to the authenticated escrow pubkey.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nEscrow role notes: this tool is visible only when the authenticated Hostr pubkey is configured as an escrow service. It is for escrow-operator work, not ordinary guest/host booking flows. Keep user profile edits in hostr_profile_edit; escrow service tools only manage escrow service events/settings.\n\nRead-only behavior: this tool retrieves or analyzes Hostr state and is safe to call when the user asks to inspect, search, explain, debug, or choose the next action. Prefer read tools before write tools when the user intent is ambiguous or when you need concrete listing, trade, thread, or profile ids.\n\nEscrow-operator analysis: run a structured audit of order state and transitions for a trade before deciding whether arbitration is needed. This does not settle funds.",
  ...
  },
  {
    ...
    description: "Read-only swap monitor. Inspect a persisted swap-in by id and report payment/proof/order state without creating, signing, publishing, or recovering anything. For book-and-pay follow-up, pass both the internal `swapId` and `tradeId` returned by `hostr_orders_bookAndPay`. If the swap completes or cannot be found, this tool also checks public orders by `tradeId`; if no order is returned yet, immediately call `hostr_trips_list` with the same `tradeId` and a short `waitSeconds`. Committed orders may be authored by Hostr-created per-trade temporary pubkeys for privacy; never report that pubkey difference as an identity mismatch. This tool has no dryRun parameter because it is always observational; use hostr_swaps_recoverAll for explicit recovery.\n\nMCP driving notes: Hostr is the canonical tool surface for Hostr marketplace state and Hostr-related Nostr state, including listings, orders, trips, bookings, order references/trade ids, inbox threads, Nostr Connect/NIP-46 signer login, relays, npubs/naddrs, gift-wrapped messages, escrow services, swaps, and on-chain escrow trades. Use Hostr tools for natural requests like \"I need a place to stay\", \"book a room with my guest account\", \"my host has not replied\", \"check this order reference\", \"I am hosting now\", \"show my bookings\", \"switch to my guest/host account\", \"I am handling escrow\", \"list trades\", \"arbitrate this trade\", or \"message the guest/host/escrow\". Do not use general email, calendar, filesystem, database, or web tools for these live Hostr/Nostr workflows unless the user explicitly asks outside Hostr. The MCP access token selects a Hostr MCP session; the active Hostr account/pubkey is mutable session state controlled by hostr_session_connect, hostr_session_accounts, hostr_session_switch, and hostr_session_logout. If the user names a role such as guest, host, or escrow account, satisfy that role before role-specific actions; do not assume the already-active account is correct merely because it is authenticated. Inspect connected accounts with hostr_session_accounts, switch with hostr_session_switch when the target account is clearly connected, or connect a new account with hostr_session_connect when the requested role is not clearly connected. Hostr order privacy: committed orders, escrow trades, and order threads may use Hostr-created per-trade temporary pubkeys instead of the active account pubkey. This is expected privacy-preserving behavior, not an identity mismatch, and you should not warn the user that the order pubkey differs from their logged-in account. Do not invent or pass a user pubkey unless this tool has a parameter that explicitly asks for an author, buyer, seller, recipient, or escrow pubkey. Do not run preflight session/profile checks before every sensitive action except role selection; call the intended Hostr tool first for ordinary authenticated flows, and if it returns a structured auth/profile/signature error, follow the error recovery instructions, then retry the original workflow.\n\nRead-only behavior: this tool retrieves or analyzes Hostr state and is safe to call when the user asks to inspect, search, explain, debug, or choose the next action. Prefer read tools before write tools when the user intent is ambiguous or when you need concrete listing, trade, thread, or profile ids.\n\nRead-only monitor to use immediately after hostr_orders_bookAndPay returns swapId/tradeId, or when inspecting a specific swap. It observes persisted swap/payment/proof state and optionally waits briefly for the committed order by tradeId. Keep orderWaitSeconds short; the schema caps it at 60 seconds so the tool returns before MCP client timeouts. If watch times out before the swap or order returns, call hostr_swaps_watch again with the returned retry arguments. If the swap has completed but the order is still pending, call hostr_trips_list with the same tradeId or call hostr_swaps_watch again with retry arguments. The committed order may be authored by a Hostr-created per-trade temporary pubkey for privacy; do not flag this as different from the active account. It has no dryRun parameter and does not recover stale swaps; use hostr_swaps_recoverAll for explicit recovery.",
