...
class StdTomlTest extends _i1.GeneratedContract {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<void> test_readToml({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[13];
    assert(checkSignature(function, '24f112b8'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> test_writeToml({
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[14];
    assert(checkSignature(function, '7ccd0406'));
    final params = [];
    return write(
      credentials,
      transaction,
