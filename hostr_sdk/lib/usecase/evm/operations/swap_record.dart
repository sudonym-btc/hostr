...
sealed class SwapRecord {
  ...
  /// The HD derivation account index used to derive the EVM key for this swap.
  ///
  /// Persisted so recovery can re-derive the correct key via
  ...
  // TODO(temp): includes `failed` so stuck swaps get retried — revert this.
...
///
/// @todo Consider persisting the RIF Relay parameters that were active when
/// the swap was created (factory address, deploy verifier, call verifier).
/// The smart-wallet address derived during swap creation is bound to the
/// factory used at that time. If the app ships a config update with new
/// contract addresses, any in-flight swaps whose lockup was sent to the
/// old-factory-derived smart wallet would become unclaimable because the
/// new factory would derive a different address. Storing the original
/// addresses here and using them during recovery would make swap-in claims
/// resilient to config changes across app releases.
class SwapInRecord extends SwapRecord {
  ...
  /// The hash of Boltz's on-chain lockup transaction.
  /// Persisted as soon as Boltz reports it via websocket, BEFORE we poll
  /// the RPC for the full [TransactionInformation].  This ensures recovery
  /// can always fetch the tx even if the app restarts during the poll.
  String? lockupTxHash;
