...
class Nwc {
  ...
  Nwc(NwcStorage nwcStorage, Ndk ndk, CustomLogger logger)
    : _nwcStorage = nwcStorage,
      _ndk = ndk,
      _logger = logger;

  Future<void> _disposeReactiveConnection(NwcCubit reactiveConnection) =>
      _logger.span('_disposeReactiveConnection', () async {
  ...
  Future<void> remove(NwcCubit reactiveConnection) => _logger.span(
    'remove',
    () async {
      _logger.i('Removing nwc connection ${reactiveConnection.url.toString()}');
      await _disposeReactiveConnection(reactiveConnection);
  ...
  Future<NwcCubit> initiateAndAdd(String url) =>
      _logger.span('initiateAndAdd', () async {
        final reactive = NwcCubit(nwc: this, logger: _logger);
  ...
  Future<NwcConnection> connect(String url) =>
      _logger.span('connect', () async {
        _logger.d('Connecting to NWC URL: $url');
        return _ndk.nwc.connect(url);
      });

  Future<GetInfoResponse> getInfo(NwcConnection connection) async {
    return _ndk.nwc.getInfo(connection);
  }

  Future<PayInvoiceResponse> payInvoice(
    NwcConnection connection,
    String invoice,
  ) => _logger.span('payInvoice', () async {
    _logger.i('Paying invoice $invoice');
    try {
      return await _ndk.nwc.payInvoice(
        connection,
        invoice: invoice,
        timeout: Duration(seconds: 20),
      );
    } catch (e, stackTrace) {
      _logger.e('Error paying invoice: $e $stackTrace');
      rethrow;
    }
  });
  ...
  Future<MakeInvoiceResponse> makeInvoice(
    ...
    int? expiry,
  }) => _logger.span('makeInvoice', () async {
    _logger.i('Making invoice for $amountSats sats');
    try {
      return await _ndk.nwc.makeInvoice(
        connection,
      ...
      _logger.e('Error making invoice: $e $stackTrace');
      rethrow;
    }
  });
