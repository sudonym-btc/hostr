...
Future<void> _runSignedInPaymentProofCase({
  ...
  final proofCompleter = Completer<Order>();
  final proofPublishCompleter = Completer<void>();
  final updatesSub = hostr.orders.updates.listen((order) {
    if (_isBuyerEscrowProofOrder(
      order: order,
      tradeId: prepared.tradeId,
      sellerPubkey: prepared.listing.pubKey,
    )) {
      if (!proofCompleter.isCompleted) {
        proofCompleter.complete(order);
      }
    }
  });
  ...
  try {
    final preparer = hostr.escrow.fund(
      EscrowFundParams(
        ...
        negotiateOrder: prepared.payableRequest,
    ...
    // The app publishes the self-signed order from this stream callback,
...
Future<_PreparedTrade> _prepareTrade({
  ...
  final firstOffer = await hostr.orderRequests.createOrderRequest(
    listing: listing,
    startDate: start,
    endDate: end,
  ...
  if (tradeId == null || tradeId.isEmpty) {
    throw StateError('Order request did not include a trade id');
  }
  ...
  if (testCase.negotiated) {
    ...
    final hostCounter = await hostr.orderRequests.createCounterOffer(
      listing: listing,
    ...
    final buyerCounter = await hostr.orderRequests.createCounterOffer(
      listing: listing,
  ...
  print(
    ...
    'requests=${thread.state.value.orderRequests.length} '
...
Future<void> _waitForThreadRequestCount({
  ...
  while (DateTime.now().isBefore(deadline)) {
    if (thread.state.value.orderRequests.length >= expectedCount) {
      return;
    }
  ...
  throw TimeoutException(
    ...
    'actualRequests=${thread.state.value.orderRequests.length}',
  );
}
...
Future<void> _expectProofReadableFromRelay({
  ...
  var lastSeen = const <Order>[];
  while (DateTime.now().isBefore(deadline)) {
    lastSeen = await hostr.orders.getByTradeId(tradeId);
    if (lastSeen.any(
      (order) =>
          _isBuyerEscrowProofOrder(
            order: order,
            tradeId: tradeId,
  ...
  throw TimeoutException(
    ...
    '$label. lastSeen=${_describeOrders(lastSeen, sellerPubkey)}',
  );
}

bool _isBuyerEscrowProofOrder({
  required Order order,
  required String tradeId,
  required String sellerPubkey,
}) {
  ...
  final threadSummary = threads
      .map(
        (thread) =>
            ...
            '${thread.state.value.orderRequests.map((request) => '${request.pubKey.substring(0, 8)}:${request.stage.name}:${request.amount}').join(",")}',
      )
      .join(' | ');
  final cachedOrders = hostr.userSubscriptions.allMyOrders$.stream.items.where(
    (order) => order.getDtag() == tradeId,
  );
  return 'threads=[$threadSummary] '
      'cachedOrders=${_describeOrders(cachedOrders, '')}';
}

String _describeOrders(Iterable<Order> orders, String sellerPubkey) {
  final values = orders
      .map(
        (order) =>
