...
/// Creates a buyer negotiate-stage order.
Order _buildNegotiate({
  ...
  return Order.create(
    ...
    quantity: 1,
    amount: DenominatedAmount(
      value: customAmount ?? listing.cost(start: start, end: end).value,
      denomination: 'BTC',
      decimals: 8,
    ),
...
/// Creates a seller-ack (commit-stage) order referencing the same
/// trade id (d-tag) as the buyer's negotiate.
Order _buildSellerAck({
  ...
  ).signAs(seller, Order.fromNostrEvent);
}
...
Order _buildCancel({
  ...
  ).signAs(signer, Order.fromNostrEvent);
}
...
void main() {
  ...
  // ─── Group 1: Pure verification via verifyGroup (no infra needed) ───────

  group('verifyGroup — order transition permutations', () {
    ...
    test('negotiate → commit → cancel (buyer) → Valid cancelled pair', () {
      ...
      final pair = OrderGroup(orders: [ack, cancelledBuyer]);
    ...
    test('negotiate → commit → cancel (seller) → Valid cancelled pair', () {
      ...
      final pair = OrderGroup(orders: [cancelledSeller, nego]);
  ...
  group('OrderGroupStatus — stage semantics', () {
    late Listing listing;
    ...
    test('negotiate-only pair has stage = negotiate', () {
      ...
      expect(pair.stage, OrderStage.negotiate);
    ...
    test('committed pair has stage = commit and isActive = true', () {
      ...
      expect(pair.stage, OrderStage.commit);
    ...
    test('cancelled pair has stage = cancel', () {
      ...
      expect(pair.stage, OrderStage.cancel);
  ...
  group(
    ...
    () {
      ...
      test('EscrowProof.validate checks tx exists on chain', () async {
        ...
        final salt = 'escrow-validate-${DateTime.now().microsecondsSinceEpoch}';
        final nego = _buildNegotiate(
          listing: listing,
          buyer: buyer,
          salt: salt,
  ...
  group('toOrderGroups + verifyGroup pipeline', () {
    ...
    test('mixed orders: valid, cancelled, and invalid pairs', () {
    ...
    test('valid zap-proof self-signed among mixed pairs → exactly 2 valid', () {
      ...
      final pairs = Orders.toOrderGroups(orders: [nego1, ack1, commit2, nego3]);
  ...
  // ─── Group 8: Order.validate — direct validation checks ─────────
