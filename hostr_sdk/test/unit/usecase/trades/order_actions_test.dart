...
Order _escrowBackedOrder(Listing listing) {
...
Order _orderWithEscrowParticipantTag(Listing listing) {
  return Order.create(
    pubKey: MockKeys.hoster.publicKey,
...
Order _cancelledNegotiationRequest(Listing listing) {
  return Order.create(
    pubKey: MockKeys.guest.publicKey,
...
void main() {
  group('TradeActionResolver', () {
    test('keeps messageEscrow action for invalid escrow-backed orders', () {
      final listing = _listing();
      final order = _escrowBackedOrder(listing);
      final invalidGroup = Invalid(
        OrderGroup(orders: [order]),
      ...
      final result = TradeActionResolver.resolve(
        ...
        ourPubkey: MockKeys.guest.publicKey,
        allOrders: const [],
        allOrdersStatus: StreamStatusLive(),
        ownOrders: [invalidGroup],
        ownOrdersStatus: StreamStatusLive(),
        payments: const [],
        paymentsStatus: StreamStatusLive(),
      );

      expect(result.availability, TradeAvailability.invalidOrder);
    ...
    test('shows messageEscrow action when escrow is known from p tag', () {
      final listing = _listing();
      final order = _orderWithEscrowParticipantTag(listing);
      final group = Valid(OrderGroup(orders: [order]));
    ...
    test('reports availability loading while overlap orders load', () {
      final listing = _listing();
      ...
      final result = TradeActionResolver.resolve(
        ...
        ourPubkey: MockKeys.guest.publicKey,
        allOrders: const [],
        allOrdersStatus: StreamStatusQuerying(),
        ownOrders: const [],
        ownOrdersStatus: StreamStatusQuerying(),
        payments: const [],
        paymentsStatus: StreamStatusLive(),
      );
