/// Unit tests for [Orders.cancel] and [Orders.confirm].
///
/// Covers:
/// - cancel() publishes a cancel-stage order with correct p-tags
/// - cancel() records a cancel transition
/// - cancel() throws when group is already cancelled
/// - confirm() publishes a commit-stage order with correct p-tags
...
/// - pre-existing order content is copied (not blank) when the caller
///   already has a order in the group
@Tags(['unit'])
library;
...
/// Build a [OrderGroup] with a buyer commit and optional seller/escrow.
OrderGroup _buildCommittedGroup({
  required String tradeId,
  KeyPair? buyerKey,
  bool includeSeller = false,
  bool includeEscrow = false,
  bool includeEscrowOrder = false,
}) {
  ...
  var group = const OrderGroup().addOrder(buyerOrder);

  if (includeSeller) {
    final sellerOrder = Order.create(
    ...
    group = group.addOrder(sellerOrder);
  }

  if (includeEscrowOrder) {
    ...
    ).signAs(MockKeys.escrow, Order.fromNostrEvent);
    group = group.addOrder(escrowOrder);
  }

  return group;
}

OrderGroup _buildNegotiateGroup({required String tradeId}) {
  final listingAnchor = _f
      .listing(
        signer: MockKeys.hoster,
        dTag: 'listing-$tradeId',
  ...
  return const OrderGroup().addOrder(buyerOrder);
}
...
void main() {
  ...
  group('Orders.cancel()', () {
    test('broadcasts a cancel-stage order', () async {
      final group = _buildCommittedGroup(tradeId: 'trade-cancel-1');

      await orders.cancel(group, MockKeys.hoster);

      expect(requests.broadcastedEvents, hasLength(1));
      final published = Order.fromNostrEvent(requests.broadcastedEvents.first);
      expect(published.stage, OrderStage.cancel);
    });

    test('preserves the tradeId (d-tag) in the published order', () async {
    ...
    test(
      ...
      () async {
        ...
        final published = Order.fromNostrEvent(
    ...
    test(
      ...
      () async {
        ...
        expect(t.type, OrderTransitionType.cancel);
        expect(t.to, OrderStage.cancel);
      },
    );
    ...
    test('also records cancel from negotiate stage', () async {
      ...
      await orders.cancel(group, MockKeys.guest);

      expect(transitions.recorded, hasLength(1));
      final t = transitions.recorded.first;
      expect(t.type, OrderTransitionType.cancel);
      expect(t.from, OrderStage.negotiate);
      expect(t.to, OrderStage.cancel);
    });
    ...
    test('throws if group is already cancelled', () async {
      ...
      // Build a new group that already has the cancel order.
      final cancelledGroup = group.addOrder(
        Order.fromNostrEvent(requests.broadcastedEvents.first),
      );

      expect(
        () => orders.cancel(cancelledGroup, MockKeys.hoster),
        throwsA(isA<Exception>()),
      );
    });

    test('sets the published order pubkey to the signer', () async {
      final group = _buildCommittedGroup(tradeId: 'trade-cancel-signer');
    ...
    test(
      ...
      () async {
        ...
        final existing = group.sellerOrder!;
  ...
  group('Orders.confirm()', () {
    test('broadcasts a commit-stage order', () async {
      final group = _buildCommittedGroup(tradeId: 'trade-confirm-1');

      await orders.confirm(group, MockKeys.escrow);

      expect(requests.broadcastedEvents, hasLength(1));
      final published = Order.fromNostrEvent(requests.broadcastedEvents.first);
      expect(published.stage, OrderStage.commit);
    });
    ...
    test(
      ...
      () async {
        ...
        expect(transitions.recorded, hasLength(1));
        final t = transitions.recorded.first;
        expect(t.type, OrderTransitionType.confirm);
        expect(t.from, OrderStage.commit);
        expect(t.to, OrderStage.commit);
      },
    );
    ...
    test(
      ...
      () async {
        final group = _buildCommittedGroup(
          ...
          includeEscrow: true,
          includeEscrowOrder: true,
        );
        final existing = group.escrowOrder!;
    ...
    test('throws if group is already cancelled', () async {
      ...
      // Add a cancel order to make it cancelled.
      final cancelOrder = Order.create(
        pubKey: MockKeys.hoster.publicKey,
      ...
      final cancelledGroup = group.addOrder(cancelOrder);

      expect(
        () => orders.confirm(cancelledGroup, MockKeys.escrow),
        throwsA(isA<Exception>()),
      );
    });
    ...
    test('throws if group is not yet committed (still negotiate)', () async {
      ...
      expect(
        () => orders.confirm(group, MockKeys.escrow),
        throwsA(isA<Exception>()),
      );
    });
    ...
    test(
      ...
      () async {
        ...
        // Confirm
        await orders.confirm(group, MockKeys.escrow);
        final confirmPublished = Order.fromNostrEvent(
          requests.broadcastedEvents.first,
        );
        ...
        // Cancel
        await orders.cancel(group2, MockKeys.escrow);
        final cancelPublished = Order.fromNostrEvent(
          requests.broadcastedEvents.first,
        );
