...
void main() {
  group('seed reservation participant proofs', () {
    test(
      ...
      () async {
        ...
        final request = trade.negotiateReservation;
        ...
        final resolved = await ReservationGroupParticipantResolver(
          keyring: KeyPairReservationParticipantKeyring(
            keyPairs: [trade.host.keyPair],
          ),
        ).resolve(ReservationGroup(reservations: [request]));
    ...
    test(
      ...
      () async {
        ...
        final trade = await helper.freshTrade();
        final request = trade.negotiateReservation;
        final hostCommit = await helper.entities.reservation(
    ...
    test('mock completed reservations use participant proofs only', () async {
      final helper = TestSeedHelper(seed: 1203);
      addTearDown(helper.dispose);

      final trade = await helper.freshTrade();
      final reservation = await helper.factory.buildMockReservation(
        trade.thread,
        hostProfile: trade.host.profile,
      );

      expect(reservation.parsedTags.participantProofs, isNotEmpty);
      expect(
        reservation.parsedTags.participantProofs.every(
          (proof) => proof.payloadHash.isNotEmpty,
        ),
        isTrue,
      );
      ...
      ).resolve(ReservationGroup(reservations: [reservation]));
