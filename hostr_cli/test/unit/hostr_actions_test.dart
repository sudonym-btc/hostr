...
void main() {
  test('action catalog publishes MCP tool names and typed input schemas', () {
    ...
    final order = HostrActionCatalog.byId('hostr.orders.negotiateOffer');
    expect(order.inputTypeName, 'HostrOrdersOfferInput');
    expect(order.inputSchema, isNot(contains('required')));
    expect(order.typescriptInput, contains('HostrOrdersOfferInput'));
  });
  ...
  test('write action inputs default to dry run and expose dryRun controls', () {
    ...
    final order = HostrOrdersOfferInput.fromJson({
      'listingAnchor': 'naddr1...',
      'start': '2026-05-04T12:00:00Z',
      'end': '2026-05-05T12:00:00Z',
    });
    expect(order.dryRun, isTrue);
    expect(order.isFollowUpOffer, isFalse);
  ...
  test(
    'order privacy descriptions do not frame temp pubkeys as mismatches',
    () {
